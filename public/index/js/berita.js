var numberOfItems = $('#colorlib-blog .col-md-6').length; 
var limitPerPage = 4; 
$('#colorlib-blog .col-md-6:gt(' + (limitPerPage - 1) + ')').hide(); 
...
$(".pagination li.page-item").on("click", function() {
  ...
  if ($(this).hasClass('active')) {
    ...
    $("#colorlib-blog .col-md-6").hide(); // Hide all items in loop, this case, all the list groups
    var grandTotal = limitPerPage * currentPage; // Get the total number of items up to the page number that was clicked on

    // Loop through total items, selecting a new set of items based on page number
    for (var i = grandTotal - limitPerPage; i < grandTotal; i++) {
      $("#colorlib-blog .col-md-6:eq(" + i + ")").show(); // Show items from the new page that was selected
    }
  }
...
$("#next-page").on("click", function() {
  ...
  if (currentPage === totalPages) {
    ...
    $("#colorlib-blog .col-md-6").hide(); // Hide all items in the pagination loop
    var grandTotal = limitPerPage * currentPage; // Get the total number of items up to the page that was selected

    // Loop through total items, selecting a new set of items based on page number
    for (var i = grandTotal - limitPerPage; i < grandTotal; i++) {
      $("#colorlib-blog .col-md-6:eq(" + i + ")").show();
